...
create table order_item_detail(
    ...
    qty int not null,
    constraint order_item_detail_item_item_id_fk foreign key (item_id) references item (item_id),
    constraint order_item_detail_order_id_fk foreign key (order_id) references order (id)
);
